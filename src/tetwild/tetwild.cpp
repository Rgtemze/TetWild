...
using std::pair;
using std::array;
typedef pair<array<double, 3>, int> CellType;
namespace tetwild {
    ...
    const Args &args, const State &state)
{
	...
	/*int count = 0;
	// keep only surface meshes
	for (int i = 0; i < tets.size(); i++) {

		for (int j = 0; j < 4; j++) {
			if (MR.is_surface_fs[i]) {
				count++;
			}
		}
	}
	logger().info("# of surfaces {}", count);*/
	...
	std::vector<double> maxs{ V_out(0,0), V_out(0,0), V_out(0,0) };
	std::vector<double> mins{ V_out(0, 0), V_out(0, 0), V_out(0,0) };

	f << "Vertices #" << V_out.rows() << std::endl;
	for (int i = 0; i < V_out.rows(); i++) {
		for (int j = 0; j < 3; j++) {
			maxs[j] = std::max(V_out(i, j), maxs[j]);
			mins[j] = std::min(V_out(i, j), mins[j]);
		}
		f << V_out(i, 0) << " " << V_out(i, 1) << " " << V_out(i, 2) << std::endl;
	}
	for (int i = 0; i < tets.size(); i++) {
		if (t_is_removed[i]) {
			continue;
	...
	f << "Tetrahedras #" << T_out.rows() << std::endl;
	for (int i = 0; i < T_out.rows(); i++) {
		f << T_out(i, 0) << " " << T_out(i, 1) << " " << T_out(i, 2) << " " << T_out(i, 3) << std::endl;
	}

	f << "Constrained Faces" << std::endl;
	std::cout << "Here we are---" << std::endl;

    for (int t = 0; t < T_out.rows(); t++) {
		std::vector<std::unordered_set<int>> us;
		for (int i = 0; i < 4; i++) {
			us.push_back(MR.tet_vertices[T_out(t, i)].on_face);
		}
		...
		for (int i = 0; i < 4; i++) {
			for (int j = i + 1; j < 4; j++) {
				for (int k = j + 1; k < 4; k++) {
					...
					for (int l : us[i]) {
						if (l < 0) continue;
						if (us[j].find(l) != us[j].end() && us[k].find(l) != us[k].end()) {
							if (found) {
	...
	f << "Neighbours" << std::endl;
	
	const static int A = 100;
	const static int B = 100;
	const static int C = 100;
	const static std::vector<int> NN{ A, B, C };
	vector<vector<vector<vector<CellType>>>> 
		grid(A + 1, 
			vector<vector<vector<CellType>>>(B + 1, 
				vector<vector<CellType>>(C + 1, 
					vector<CellType>())));

	for (int t = 0; t < T_out.rows(); t++) {
		std::vector<int> v_ids{ T_out(t, 0), T_out(t, 1), T_out(t, 2), T_out(t, 3) };
		std::vector<std::array<double, 3>> vertices;
		vertices.push_back({ V_out(v_ids[0], 0), V_out(v_ids[0], 1), V_out(v_ids[0], 2) });
		vertices.push_back({ V_out(v_ids[1], 0), V_out(v_ids[1], 1), V_out(v_ids[1], 2) });
		vertices.push_back({ V_out(v_ids[2], 0), V_out(v_ids[2], 1), V_out(v_ids[2], 2) });
		vertices.push_back({ V_out(v_ids[3], 0), V_out(v_ids[3], 1), V_out(v_ids[3], 2) });
	...
	/*for (int i = 0; i < T_out.rows(); i++) {
		...
		vertices.push_back(T_out(i, 0));
		vertices.push_back(T_out(i, 1));
		vertices.push_back(T_out(i, 2));
		vertices.push_back(T_out(i, 3));

		std::vector<std::vector<int>> tmp{ { vertices[1], vertices[2], vertices[3]},
									   {vertices[0], vertices[2], vertices[3]},
									   {vertices[0], vertices[1], vertices[3]},
									   {vertices[0], vertices[1], vertices[2]},
		};
		...
		for (int t = 0; t < 4; t++) {
			std::sort(tmp[t].begin(), tmp[t].end());

			bool flag = false;
			for (int j = 0; j < T_out.rows(); j++) {
				if (i == j) continue;

				std::vector<int> vertices2;
				vertices2.push_back(T_out(j, 0));
				vertices2.push_back(T_out(j, 1));
				vertices2.push_back(T_out(j, 2));
				vertices2.push_back(T_out(j, 3));
				std::sort(vertices2.begin(), vertices2.end());


				if (std::includes(vertices2.begin(), vertices2.end(), tmp[t].begin(), tmp[t].end())) {
					f << j << " ";
					flag = true;
					break;
				}
...
Eigen::MatrixXd eign1(std::vector<Point_3> data)
{
	Eigen::MatrixXd eMatrix(data.size(), 3);
	for (int i = 0; i < data.size(); ++i) {
		eMatrix(i, 0) = CGAL::to_double(data[i][0]);
		eMatrix(i, 1) = CGAL::to_double(data[i][1]);
		eMatrix(i, 2) = CGAL::to_double(data[i][2]);
	}
		
	return eMatrix;
}

Eigen::MatrixXi eign2(std::vector<std::array<int, 3>> data)
{
	Eigen::MatrixXi eMatrix(data.size(), 3);
	for (int i = 0; i < data.size(); ++i) {
		eMatrix(i, 0) = data[i][0];
		eMatrix(i, 1) = data[i][1];
		eMatrix(i, 2) = data[i][2];
	}
    ...
    std::vector<std::array<int, 3>> &m_faces)
{
	...
	igl::writeOBJ("ziyaerkoc.obj", eign1(m_vertices), eign2(m_faces));
	/*for (int i = 0; i < VI.rows(); i++) {
		m_vertices.push_back({VI(i, 0), VI(i, 1),VI(i, 2) });
	}

	for (int i = 0; i < FI.rows(); i++) {
		m_faces.push_back({ FI(i, 0), FI(i, 1), FI(i, 2) });
	}*/
